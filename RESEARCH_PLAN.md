...
- **Extended macro controls** (mirrors the runnable code): population size (for exposure + checks), youth unemployment rate, urban population share, and GDP per capita (PPP) to soak up business-cycle and urbanization shocks that might confound inequality effects.
